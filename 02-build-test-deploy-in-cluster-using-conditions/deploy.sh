...
# Create Pipeline
kubectl $OPERATION -f pipelines/build-test-deploy-in-cluster-using-conditions.yaml


# Run OpenWhisk Pipeline for NodeJS App after replacing DOCKER_USERNAME with user specified name
sed -e 's/${DOCKER_USERNAME}/'"$DOCKER_USERNAME"'/' 02-build-test-deploy-in-cluster-using-conditions/pipelinerun.yaml.tmpl > 02-build-test-deploy-in-cluster-using-conditions/pipelinerun.yaml
kubectl $OPERATION -f 02-build-test-deploy-in-cluster-using-conditions/pipelinerun.yaml
if [ "$OPERATION" = "$DELETE" ]; then
  rm 02-build-test-deploy-in-cluster-using-conditions/pipelinerun.yaml
  kubectl $OPERATION deployment.apps/cdcon-hello-app-2-1
  kubectl $OPERATION service/cdcon-hello-app-2-1
fi
