...
if [ "$OPERATION" != "$DELETE" ]; then
kubectl $OPERATION -f workspace/cdcon-workspace.yaml
fi
...
# Create Pipeline
kubectl $OPERATION -f pipeline/pipeline.yaml

# Run OpenWhisk Pipeline for NodeJS App after replacing DOCKER_USERNAME with user specified name
sed -e 's/${DOCKER_USERNAME}/'"$DOCKER_USERNAME"'/' pipelinerun/pipelinerun.yaml.tmpl > pipelinerun/pipelinerun.yaml
kubectl $OPERATION -f pipelinerun/pipelinerun.yaml
if [ "$OPERATION" = "$DELETE" ]; then
  rm pipelinerun/pipelinerun.yaml
fi
